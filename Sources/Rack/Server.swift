...
extension Rack {
  ...
  class Server {
    ...
    //  )
    //
    // Further options available here are documented on Rack.Server.init
    static func start(options: [:] = nil) {
      init(options).start()
    }
    ...
    static func logging_middleware() {
      lambda { |server|
        server.server.name =~ /CGI/ || server.options[:quiet] ? nil : [Rack.CommonLogger, $stderr]
      }
    }

    static func default_middleware_by_environment() {
      m = Hash.new {|h,k| h[k] = []}
      m["deployment"] = [
        [Rack.ContentLength],
        [Rack.Chunked],
    ...
    private
      func build_app_and_options_from_config() {
